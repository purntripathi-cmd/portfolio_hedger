...
# Live Data Fetcher via yfinance with Fallback
...
with h1:
    ...
    st.caption(f"Downside Insurance Optimizer & Zerodha Paper Trading ({data_status} Feed)")
with h2:
    st.metric(
        label="NIFTY 50 (NSE Benchmark)", 
        value=f"{nifty_spot:,.2f}", 
        delta=f"{nifty_chg_pct:+.2f}% Today",
        help="Official NSE Nifty 50 Index CMP and percentage change from previous day's close."
    )
with h3:
    ...
    st.metric(
        ...
        delta=sentiment_tag, 
        delta_color="inverse",
        help="India VIX measures annualized expected 30-day volatility. VIX < 13 = Low/Cheap premiums; 13–16.5 = Balanced; > 16.5 = High/Elevated option pricing."
    )
with h4:
    st.write("")
    if st.button("🔄 Refresh CMP", use_container_width=True, help="Refreshes live market price ticks and recomputes paper trades MTM"):
        st.cache_data.clear()
        st.rerun()
...
with tab_advisor:
    ...
    comparison_data = [
        {
            ...
            "Absorbed Market Cost (Deductible)": f"{downside_tol:.1f}% ({format_inr_in_words(absorbed_rupees)})",
            ...
            "Max Cash Loss on Hedge": f"₹ {s1_charges['net_cost']:,.2f} (Capped)"
        },
        {
            ...
            "Max Cash Loss on Hedge": f"₹ {(s2_net_prem * total_qty) + s2_total_charges:,.2f} (Capped)"
        },
        {
            ...
            "Max Cash Loss on Hedge": f"₹ {(s3_net_prem * total_qty) + s3_total_charges:,.2f} (Capped)"
        }
    ]
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Absorbed Market Cost (Deductible)": st.column_config.TextColumn("Absorbed Market Cost (Deductible)", help="The first tranche of portfolio loss you absorb out-of-pocket before insurance triggers."),
            "Portfolio Drag (%)": st.column_config.TextColumn("Portfolio Drag (%)", help="Net percentage cost of option insurance relative to your total portfolio. Deducted if market stays flat or rallies."),
            "Zerodha Friction (₹)": st.column_config.TextColumn("Zerodha Friction (₹)", help="Brokerage, STT, NSE Exchange fees, SEBI charges, Stamp duty, GST."),
            "Downside Protection": st.column_config.TextColumn("Downside Protection", help="Coverage floor provided during severe drawdowns."),
            "Max Cash Loss on Hedge": st.column_config.TextColumn("Max Cash Loss on Hedge", help="The maximum capital lost on the hedge if the market reaches new all-time highs.")
        }
    )

    # Interactive Market Drop Simulator
    st.markdown("---")
    st.subheader("🧪 Live Drawdown & Tax Impact Simulator")
    sim_col1, sim_col2 = st.columns([2, 4])
    with sim_col1:
        sim_drop = st.slider("Simulate Market Crash (% Drop)", min_value=0.0, max_value=25.0, value=5.0, step=0.5)
        sim_strat = st.selectbox("Select Strategy to Simulate", ["1. OTM Protective Put", "2. Bear Put Spread", "3. Zero-Cost Collar"])
    
    with sim_col2:
        sim_port_loss = portfolio_val * (sim_drop / 100.0) * portfolio_beta
        sim_nifty_end = nifty_spot * (1.0 - (sim_drop / 100.0))
        
        if "Protective Put" in sim_strat:
            hedge_payoff = max(0.0, (otm_strike_primary - sim_nifty_end)) * total_qty
            net_hedge_pnl = hedge_payoff - s1_charges["net_cost"]
        elif "Spread" in sim_strat:
            p1 = max(0.0, (otm_strike_primary - sim_nifty_end)) * total_qty
            p2 = max(0.0, (otm_strike_lower - sim_nifty_end)) * total_qty
            net_hedge_pnl = (p1 - p2) - ((s2_net_prem * total_qty) + s2_total_charges)
        else:
            p1 = max(0.0, (otm_strike_primary - sim_nifty_end)) * total_qty
            c1 = max(0.0, (sim_nifty_end - otm_call_strike)) * total_qty
            net_hedge_pnl = (p1 - c1) - ((s3_net_prem * total_qty) + s3_total_charges)

        overall_net_pnl = -sim_port_loss + net_hedge_pnl

        m1, m2, m3 = st.columns(3)
        m1.metric("Unhedged Portfolio Loss", f"-₹ {sim_port_loss:,.2f}", help="Rupee drop in your stock/MF holdings.")
        m2.metric("Option Hedge P&L (Net)", f"{'+' if net_hedge_pnl>=0 else ''}₹ {net_hedge_pnl:,.2f}", help="Cash gain from option contract after Zerodha fees.")
        m3.metric("Net Portfolio Position", f"{'+' if overall_net_pnl>=0 else ''}₹ {overall_net_pnl:,.2f}", delta=f"Protected: ₹{max(0.0, net_hedge_pnl):,.2f}")
        st.caption("ℹ️ *Tax Treatment: Derivative P&L is categorized as Non-Speculative Business Income (taxed at slab rate); portfolio paper losses do not trigger capital loss until shares are sold.*")
    ...
    with col_exec1:
        trade_strat = st.selectbox(
            ...
            help="Select which hedging structure to log into your persistent paper trading book."
        )
    with col_exec2:
        st.write("")
    ...
    if df_trades.empty:
        ...
        for idx, row in df_trades.iterrows():
            if row["status"] == "ACTIVE":
                ...
                # Dynamic drift based on difference between entry strike and live spot
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "timestamp": st.column_config.TextColumn("Timestamp", help="Date and time when position was initiated."),
                ...
                "cmp": st.column_config.NumberColumn("CMP (₹)", format="₹ %.2f", help="Current live market price per unit based on real-time Nifty movements."),
...
with tab_analytics:
    ...
    kp1.metric("Total Paper Hedges", total_trades, help="Total paper hedges deployed.")
    kp2.metric("Active Insurances", active_hedges, help="Active open option hedge contracts.")
    kp3.metric("Total Friction (Zerodha)", f"₹ {total_friction_paid:,.2f}", help="Cumulative brokerage, STT, and taxes paid.")
    kp4.metric("Net Realized P&L", f"₹ {net_realized:,.2f}", delta=f"{net_realized:,.2f}", help="Locked-in P&L from closed hedges.")
    kp5.metric("Unrealized MTM", f"₹ {net_unrealized:,.2f}", delta=f"{net_unrealized:,.2f}", help="Floating MTM P&L of active open contracts.")
